...
def test_extract_mutation_namedtuples_from_sequences7():
    ...
    # wrong input type for variant_seq_str -> str
    with pytest.raises(TypeError):
        me.extract_mutation_namedtuples_from_sequences(
            variant_seq_str="AAAA", parent=parent_sequence
        )

    # wrong input type for parent -> list
    with pytest.raises(TypeError):
        me.extract_mutation_namedtuples_from_sequences(
            variant_seq_str=variant_sequences, parent=[parent_sequence]
        )


def test_extract_mutation_str_from_sequences():
    parent_sequence = "ATCGATCG"
    variant_sequences = ["ATCGTTCG", "ATTGATCG", "ATCGATCC"]
    offset = 0
    mut = me.extract_mutation_str_from_sequences(
        variant_sequences, parent_sequence, offset
    )
    assert mut == ["A5T", "C3T", "G8C"]

    offset = 1
    mut = me.extract_mutation_str_from_sequences(
        variant_sequences, parent_sequence, offset
    )
    assert mut == ["A4T", "C2T", "G7C"]

    offset = -1
    mut = me.extract_mutation_str_from_sequences(
        variant_sequences, parent_sequence, offset
    )
    assert mut == ["A6T", "C4T", "G9C"]

    offset = 0
    mut = me.extract_mutation_str_from_sequences(
        [parent_sequence], parent_sequence, offset, empty_is_wt=True
    )
    assert mut == ["wt"]
