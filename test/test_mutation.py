...
import pytest
from pedata.mutation.mutation import (
    Mutation,
    get_missing_values,
    fill_all_missing_sequences,
...
def test_sort_mutations_by_pos():
...
def test_parse_variant_mutations():
...
def test_parse_all_mutations():
    ...
    dataset = ds.Dataset.from_dict({"aa_seq": ["MAETK", "GMAFT"], "target foo": [1, 2]})
    ...
    dataset = ds.Dataset.from_dict(
        {"dna_seq": ["ACGTCGT", "ACGTCGA"], "target foo": [1, 2]}
    )
    parsed_mutations, non_parsed_idx = Mutation.parse_all_mutations(dataset)
    assert parsed_mutations == [] and non_parsed_idx == []
    ...
    parsed_mutations, non_parsed_idx = Mutation.parse_all_mutations(dataset)
    assert (
        parsed_mutations == [[(32, "L", "T"), (33, "G", "C")]] and non_parsed_idx == []
    )
    ...
    parsed_mutations, non_parsed_idx = Mutation.parse_all_mutations(dataset, offset=1)
    assert (
        parsed_mutations == [[(33, "L", "T"), (34, "G", "C")]] and non_parsed_idx == []
    )
...
def test_combine_variant_mutations():
...
def test_generate_variant_mutation_combinations():
    ...
    result = Mutation.generate_variant_mutation_combinations(
        variants1,
        variants2,
    )
    ...
    # Test Case 6: 3 variants
    variants1 = [Mut(1, "A", "G"), Mut(1, "A", "T")]
    variants2 = [Mut(3, "G", "C"), Mut(3, "G", "A")]
    variants3 = [Mut(4, "T", "C"), Mut(4, "T", "A")]
...
def test_generate_variant_mutation_combinations_within_dataset():
    ...
    mut = [Mut(13, "G", "M")]
    combined_mutations = Mutation.generate_variant_mutation_combinations_within_dataset(
        mut
    )
    ...
    # Test Case 5: Mutiple mutations which do no make sense
    with pytest.raises(TypeError):
        mut = [Mut(1, "A", "G"), Mut(1, "A", "T"), Mut(1, "A", "C"), Mut(1, "A", "P")]
        _ = Mutation.generate_variant_mutation_combinations_within_dataset(mut)


def test_concat_mutations():
...
def test_get_parent_sketch_from_mutations():
...
def test_estimate_offset():
    ...
    with pytest.raises(Exception):
        Mutation.estimate_offset(dataset, parent=parent_seq, only_perfect_matches=True)[
            "offset"
        ][0] == 0
    ...
    with pytest.raises(KeyError):
        Mutation.estimate_offset(dataset, parent=parent_seq)["offset"][0]  # Returns -55
    ...
    assert Mutation.estimate_offset(dataset, parent=parent_seq, most_likely=True) == 1
...
def test_estimate_offset_ambiguous():
...
def test_apply_variant_mutations():
...
def test_apply_all_mutations():
    ...
    empty_dataset = ds.Dataset.from_dict({"aa_mut": [], "aa_seq": [], "target foo": []})
    with pytest.raises(ValueError):
    ...
    dataset = ds.Dataset.from_dict(
        {
            "aa_mut": ["wildtype", "C2H_E4Y", "F5T"],
            "aa_seq": ["ACDEFGH", None, None],
            "target foo": [1, 2, 3],
        }
    )
    mutated_sequences = Mutation.apply_all_mutations(dataset)
    assert mutated_sequences == ["ACDEFGH", "AHDYFGH", "ACDETGH"]
    ...
    dataset = ds.Dataset.from_dict(
        {
            "dna_mut": ["wildtype", "G3A", "C2T"],
            "dna_seq": ["ACGTAGT", None, None],
            "target foo": [1, 2, 3],
        }
    )
    Mutation.apply_all_mutations(dataset) == ["MAMPS", "MTKPS"]
...
def test_fill_all_missing_sequences():
    dataset_filled = fill_all_missing_sequences(
        Dataset.from_dict(aa_example_0_missing_val)
    )
    assert dataset_filled["aa_seq"] == ["GMPKSEFTHC", "GMPKSEFMHC", "GMGKSEFTHC"]

    dataset_filled = fill_all_missing_sequences(
        Dataset.from_dict(dna_example_1_missing_val)
    )
    assert dataset_filled["dna_seq"] == ["GCTCCG", "GCTCCT", "GCGCCG"]
