...
from ..encoding.alphabets import padding_value_enc
...
def seq_strings_to_array(
    seq_strings: onp.ndarray | list[str], pad=False, pad_char="?"
) -> onp.array:
    """Convert strings into an array.
    If needed a padding can be added.

    Args:
        seq_strings: List or array of sequences
        pad: if padding should be done (default is False)
        pad_char:  padding character  (default is '?')
...
def unirep(df: ds.Dataset | pd.DataFrame) -> dict[str, onp.ndarray]:
    """Add unirep encoding to a dataframe.

    Args:
        df: Data Frame or data set to which encodings should be added

    Returns:
        rval: DataFrame with new encodings added
    """
    h_avg, h_final, _ = get_reps(df["aa_seq"])
