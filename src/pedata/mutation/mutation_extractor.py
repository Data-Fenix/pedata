...
def extract_mutation_namedtuples_from_sequences(
    ...
    # Validate input
    if not isinstance(variant_seq_str, list):
        raise TypeError(
...
def extract_mutation_str_from_sequences(
    ...
    # set offset to 0 now that it has already been corrected
    offset = 0

    # Convert mutations to string
    encoded_mut = convert_all_variant_mutations_to_str(
        mutations, delimiting_char, offset, empty_is_wt
    )
