...
# pedata
from ..integrity import check_dataset_format, check_mutation_namedtuple
...
class Mutation:
    ...
    def parse_all_mutations(
        ...
        if "mut_column" in locals():
            ...
            for row_idx, var in enumerate(dataset[mut_column]):
                ...
                if var is None or var.strip().lower() in [
                    "wildtype (wt)",
                    "wildtype",
                    "",
                    "wt",
    ...
    @staticmethod
    def generate_variant_mutation_combinations(*variants: list[Mut]) -> list[list[Mut]]:
        """
        Generate all possible combinations of mutations from multiple sets of variants.
        The function pairs each variant from one set with every variant from the other sets.
        The resulting combined mutations are returned as a list of Mut namedtuples.

        Args:
            *variants (list[Mut]): Variable number of lists of mutations representing different sets of variants.

        Returns:
            list[list[Mut]]: List of combined mutations representing all possible combinations.

        Example:
            >>> from pedata.constants import Mut
            >>> variants1 = [Mut(32, 'L', 'T'), Mut(33, 'G', 'C')]
            >>> variants2 = [Mut(13, 'K', 'M')]
            >>> variants3 = [Mut(45, 'A', 'G')]
            >>> results = generate_variant_mutation_combinations(variants1, variants2, variants3)
            >>> print(results) # doctest: +NORMALIZE_WHITESPACE
            [[Mut(pos=32, src='L', targ='T'), Mut(pos=13, src='K', targ='M'), Mut(pos=45, src='A', targ='G')], #FIXME CHECKTHAT
            [Mut(pos=33, src='G', targ='C'), Mut(pos=13, src='K', targ='M'), Mut(pos=45, src='A', targ='G')]]
        """

        # Check for valid lists
        if any(not isinstance(variant_list, list) for variant_list in variants):
            raise TypeError("Invalid input: Expected valid lists for variants")

        # Check for non-empty lists
        if any(len(variant_list) == 0 for variant_list in variants):
            raise ValueError(
                "Invalid input: Expected non-empty lists of variant mutations"
            )

        # Initialize the combined mutations with the Cartesian product of the sets
        combined_mutations = [[]]

        # Generate combinations of mutations from the sets of variants
        for variant_list in variants:
            combined_mutations = [
                combination + [variant]
                for combination in combined_mutations
    ...
    def generate_variant_mutation_combinations_within_dataset(
        ...
        # check if the mutations are valid ##FIXME this is not a good way to check for validity  -
        for mut in rval:
            for m in mut:
                check_mutation_namedtuple(m)
    ...
    def concat_mutations(mut: list[list[Mut]]) -> list[Mut]:
        ...
        for m in mut:
            if not isinstance(m, list):
                raise TypeError(
                    "Mutations should be a list of lists before concatenating"
                )
            rval.extend(m)
    ...
    def get_parent_aa_seq(
        ...
        wildtypes = [
            i
            for i, v in enumerate(dataset[mut_column])
            if v is not None and ("wildtype" in v.lower() or "wt" in v.lower())
        ]
        if len(wildtypes) != 1:
            raise ValueError(
    ...
    def estimate_offset(
        ...
        if not most_likely:
            ...
            if len(rval) > 1:
                raise ValueError(
                    f"When processing mutation codes, multiple offsets seem to be plausible: {rval['offset']}. Cannot automatically decide for an offset."
                )
    ...
    def apply_all_mutations(
        ...
        if isinstance(mutations, ds.Dataset):
            # ======# FIXME create a function fo this, it is used in multiple places
            existing_columns = set(mutations.features.keys())
            ...
            if (
                parent is None
            ):  # FIXME this will break in case of dna dataset without parent sequence
        ...
        for row_idx in range(len(mutations)):
            # Append whatever is in the aa_seq column when there are no mutations:
            # - this is the case when the mutation column is empty but the sequence column is filled
            # - this is the case for the wildtype line
            if row_idx in no_mutation:
                rval.append(dataset[seq_column][row_idx])

            # Apply variant mutations to the parent sequence
            # FIXME IS: But this is probably incorrect in general now that we allow more than one sequence without a mutation code.
...
def fill_all_missing_sequences(
    dataset: Dataset, mutation_code_offset: int = None
) -> Dataset:
    """fill all fillable missing sequences in the dataset
    Args:
    ...
    if "aa_seq" in dataset.column_names:
        ...
        dataset = fill_missing_sequences(
            dataset, "aa_seq", mutation_code_offset=mutation_code_offset
        )

    elif "dna_seq" in dataset.column_names:
        # Check for missing values in the 'dna_seq' column
        dataset = fill_missing_sequences(
            dataset, "dna_seq", mutation_code_offset=mutation_code_offset
        )
...
def fill_missing_sequences(
    dataset: Dataset, feature: str, mutation_code_offset: int = None
) -> Dataset:
    ...
    Args:
        ...
        mutation_code_offset: offset to apply to the missing values
            Defaults to None, meaning that the mutation_code_offset will be determined automatically through the mutation code.
    ...
    if missing_values.sum() > 0:
        ...
        df.loc[missing_values, feature] = pd.DataFrame(
            {
                feature: Mutation.apply_all_mutations(
                    dataset, offset=mutation_code_offset
                )
            }
